    ...
    var _showTable = function() {
        var datatable = $('#kt_datatable_team').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
                        url: '/api/datatables/team/'+_getTeamId(),
                    },
                },
                ...
                template: function(row) {
                    return '\
							<div class="progress progress-xs mx-3 w-100">\
								<div id="team-progressbar" class="progress-bar bg-success" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>\
							</div>\
						';
                }
            },
            {
                ...
                width: 30,
                template: function(row) {
                    return '<span id="team-percent" class="font-weight-bolder text-dark">0%</span>';
                }
            },
            {
                field: 'Actions',
...
//Team Objective Datatable
var OkrsListDatatable = function() {
    // Private functions
    var objNo = 0;
	var objArray = new Array();
	var sumTotalPercentage = 0;
        ...
        var datatable = $('#kt_datatable_okr').KTDatatable({
            ...
            },
            {
                ...
                template: function(row) {
					...
					sumTotalPercentage += row.percentage;
					var avgPercent = Math.floor(sumTotalPercentage/objNo);

					$('#team-progressbar').css('width', avgPercent+'%');
					$('#team-percent').text(avgPercent+'%');
				...
				columns: [
					{
						...
						template: function(data) {
							...
							return '<div contenteditable="false">'+data.krName+'</div>';
						}
					}, {
						field: 'progress',
						title: 'PROGRESS',
...
//Member datatable
var MembersListDatatable = function() {
	//private variables
	var memberDataArray = new Array();
	
	// Private functions
	// Member table
	var _showTable = function() {
		var datatable = $('#kt_datatable_member').KTDatatable({
			// datasource definition
			data: {
				type: 'remote',
				source: {
					read: {
						url: '/api/datatables/team/members',
						params : {
							teamId : _getTeamId(),
						},
					},
			...
			columns: [
				...
				},
				{
					...
					title: '%',
					template: function(data) {
						return '<span class="font-weight-bolder text-dark">0%</span>';
					},
				},
				{
					field: 'Actions',
					...
					template: function(row) {
						return '\
							<a href="/okr?id=' + row.id + '" class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2">\
								<span class="svg-icon svg-icon-md">\
									<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1" class="svg-icon">\
										<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
											<rect x="0" y="0" width="24" height="24"/>\
		...
		datatable.on('click', '[data-record-id]', function() {
			//console.log($(this).data('record-id'));
			//console.log(memberDataArray[id]);
			
			//member id
			var id = $(this).data('record-id');
			
			//set form field values 
			$('#edit-name-input').val(memberDataArray[id].name);
			$('#edit-member-id-input').val(memberDataArray[id].userId);
			$('#edit-email-input').val(memberDataArray[id].email);
			$('#edit-team-id-select').val(memberDataArray[id].team.id);
			$('#edit-level-select').val(memberDataArray[id].level);
			
			//a function to handle a form that edit a member
			_handleFormEditMember($(this).data('record-id'));
			
			//show a member edit modal form
            $('#kt_datatable_edit_member_modal').modal('show');
		});
	...
	//a function to handle adding a member
	var _handleFormAddMember = function() {
		var form = KTUtil.getById('add_member_form');
		//var formSubmitUrl = KTUtil.attr(form, 'action');
		var formSubmitButton = KTUtil.getById('add_member_form_submit');
		...
		FormValidation
		    ...
		    .on('core.form.valid', function() {
		        ...
		        FormValidation.utils.fetch('/member/add', {
                    method: 'POST',
                    dataType: 'json',
		            params: {
						name: $('#add-member-name-input').val(),
						userId: $('#add-member-id-input').val(),
						email: $('#add-email-input').val(),
						password: $('#add-password-input').val(),
						teamId: _getTeamId,
						level: $('#add-level-select').val(),
		            },
		        }).then(function(response) { // Return valid JSON
					// Release button
					KTUtil.btnRelease(formSubmitButton);
	...
	//a function to handle editing a member
    var _handleFormEditMember = function(id) {
		var form = KTUtil.getById('edit_member_form');
		//var formSubmitUrl = KTUtil.attr(form, 'action');
		var formSubmitButton = KTUtil.getById('edit_member_form_submit');
		...
		FormValidation
		    ...
		    .on('core.form.valid', function() {
		        ...
		        FormValidation.utils.fetch('/member/edit', {
                    method: 'POST',
                    dataType: 'json',
		            params: { 
                        id: id,
                        name: $('#edit-member-name-input').val(),
						userId: $('#edit-member-id-input').val(),
						email: $('#edit-email-input').val(),
						password: $('#edit-password-input').val(),
						teamId: _getTeamId,
						level: $('#edit-level-select').val(),
		            },
		        }).then(function(response) { // Return valid JSON
					// Release button
					KTUtil.btnRelease(formSubmitButton);
