...
var MembersListDatatable = function() {
	...
	var _showTable = function() {
		var datatable = $('#kt_datatable_member').KTDatatable({
			...
			columns: [
					...
					template: function(row) {
						return '\
							<div class="progress progress-xs mx-3 w-100">\
								<div id="member-progressbar" class="progress-bar bg-success" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>\
							</div>\
						';
					},
				},
				{
					...
					title: '%',
					template: function(row) {
						return '<span id="member-percent" class="font-weight-bolder text-dark">0%</span>';
					},
				},
				{
					field: 'Actions',
		...
		datatable.on('click', '[data-record-id]', function() {
			...
			//call a function to handle editing a member
			_handleFormEditMember($(this).data('record-id'));
			
			//show a member edit form modal 
            $('#kt_datatable_edit_member_modal').modal('show');
		});
		
		//Trigger when the datatable ajax is completed
        datatable.on(
            'datatable-on-ajax-done',
            function(e) {
				//Initialize Objectives Datatables
    ...
    var _showTable = function() {
        var datatable = $('#kt_datatable_okr').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
						url: '/api/datatables/member/okrs',
						params: {
							memberId: _getId(1),
							level: 'M',
							quarter: 4,
            ...
            },
            {
                ...
                template: function(row) {
					...
					$('#member-progressbar').css('width', avgPercent+'%');
					$('#member-percent').text(avgPercent+'%');
        ...
        datatable.on('click', '[data-record-id]', function() {
			...
			//call an Objective Form Edit Handler function
			_handleFormEditOkr($(this).data('record-id'));
			
			//show Objective Edit Form Modal
            $('#kt_datatable_edit_okr_modal').modal('show');
		});

		//Member Objective's Key Results Datatables
...
//get member ID and team ID from URL
var _getId = function(last){
