...
function renderImagesOnList(data, field, path, prop) {
    ...
    for (let i = 0; i < Math.min(listData.length, visibleImages); i++) {
        result +=
            `<a class="symbol symbol-30 symbol-circle" data-toggle="tooltip" title="${listData[i].name}" href="${prop !== undefined ? path +'='+ listData[i][prop]: '#'}">
                 <img class="" src="${getImagesUrl(listData[i].image)}" alt="photo">
             </a>`;
    }
